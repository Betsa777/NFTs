// SPDX-License-Identifier: SEE LICENSE IN LICENSE
pragma solidity ^0.8.18;
import {Test, console} from "forge-std/Test.sol";
import {DeployMoodNft} from "../../script/DeployMoodNft.s.sol";

contract DeployMoodNftTest is Test {
    DeployMoodNft public deployer;
    string public happyImageToUri =
        "data:image/svg+xml;base64,";
    string public sadImageToUri =
        "data:image/svg+xml;base64,";

    string public svgHappy;
    string public svgSad;

    function setUp() public {
        //Il s'agit d'un unit test ici parce qu'on teste le deployer de MoodNft.sol
        deployer = new DeployMoodNft();
        svgHappy = vm.readFile("./images/dynamicNft/happy.svg");
        svgSad = vm.readFile("./images/dynamicNft/sad.svg");
    }

    function testConvertSvgToUri() public view {
        // vm.prank(msg.sender);
        string memory resultHappy = deployer.svgToImageUri(svgHappy);
        string memory resultSad = deployer.svgToImageUri(svgSad);
        console.log("happy is :", happyImageToUri);
        console.log("sad is :", sadImageToUri);
        assertEq(
            keccak256(abi.encodePacked(resultHappy)),
            keccak256(abi.encodePacked(happyImageToUri))
        );
        assertEq(
            keccak256(abi.encodePacked(sadImageToUri)),
            keccak256(abi.encodePacked(resultSad))
        );
    }
}
