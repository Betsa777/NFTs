// SPDX-License-Identifier: SEE LICENSE IN LICENSE
pragma solidity ^0.8.18;
import {Test} from "forge-std/Test.sol";
import {console} from "forge-std/console.sol";
import {MoodNft} from "../../src/MoodNft.sol";

contract MoodNftTest is Test {
    MoodNft moodNft;
    string public happyImageToUri =
        "data:image/svg+xml;base64,";
    string public sadImageToUri =
        "data:image/svg+xml;base64,";

    function setUp() public {
        //quand on utilise pas de deployer il s'agit d'un unit test
        moodNft = new MoodNft(sadImageToUri, happyImageToUri);
    }

    function testViewTokenUri() public {
        vm.startPrank(msg.sender);
        moodNft.mintNft();
        string memory imageUriToUrl = moodNft.tokenURI(0);
        string memory expectedResult = string(
            abi.encodePacked(happyImageToUri)
        );
        moodNft.flipMood(0);
        moodNft.tokenURI(0);
        vm.stopPrank();
        //console.log(imageUriToUrl);
        //console.log("expected Result is ,", expectedResult);

        // assert(
        //     keccak256(abi.encodePacked(ImageUriToUrl)) ==
        //         keccak256(abi.encodePacked(expectedResult))
        // );
    }
}
